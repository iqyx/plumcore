...
/* Boot key is generated by hashing a UTF-8 string "boot-key" with SHA256 and cropping it to 32 bits.
 * It can be any random number. */
#define BOOT_FLAG_KEY 0x791b8252
...
chainloader_ret_t chainloader_boot(ChainLoader *self) {
	...
	boot_flags.entry_point = self->elf.elf_header.entry;
	boot_flags.vtor = self->vector_table;
	/* A randomly generated key */
	boot_flags.key = BOOT_FLAG_KEY;
...
void __attribute__((constructor)) chainloader_check_and_jump(void) {
	if (boot_flags.key == BOOT_FLAG_KEY) {
		/* Do not cycle here. */
		boot_flags.key = 0x0;

		register uint32_t msp __asm("msp");
		typedef void (*app_entry_t)(void);
		app_entry_t app_entry = (app_entry_t)boot_flags.entry_point;

		/* Set stack pointer, relocate the vector table to the new position and run the application firmware. */
		msp = *(uint32_t *)boot_flags.vtor;
		SCB_VTOR = boot_flags.vtor;
		app_entry();
		(void)msp;
	}
