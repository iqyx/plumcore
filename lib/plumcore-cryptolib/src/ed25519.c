...
static void U64TO8_LE(unsigned char *p, const uint64_t v) {
	p[0] = (unsigned char)(v      );
	p[1] = (unsigned char)(v >>  8);
	p[2] = (unsigned char)(v >> 16);
	p[3] = (unsigned char)(v >> 24);
 ...
 */
static int ed25519_compare(const unsigned char *x, const unsigned char *y, size_t len) {
...
static void curve25519_add_after_basic(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	uint32_t c;
...
static void curve25519_add_reduce(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	uint32_t c;
...
static void curve25519_sub_after_basic(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	uint32_t c;
...
static void curve25519_sub_reduce(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	uint32_t c;
...
static void curve25519_expand(bignum25519 out, const unsigned char in[32]) {
	...
	if (endian_check.s == 1) {
		x0 = *(const uint32_t *)(in + 0);
		x1 = *(const uint32_t *)(in + 4);
		x2 = *(const uint32_t *)(in + 8);
		x3 = *(const uint32_t *)(in + 12);
		x4 = *(const uint32_t *)(in + 16);
		x5 = *(const uint32_t *)(in + 20);
		x6 = *(const uint32_t *)(in + 24);
		x7 = *(const uint32_t *)(in + 28);
	} else {
		#define F(s)                         \
			((((uint32_t)in[s + 0])      ) | \
			 (((uint32_t)in[s + 1]) <<  8) | \
...
/* out = (flag) ? in : out */
static void curve25519_move_conditional_bytes(uint8_t out[96], const uint8_t in[96], uint32_t flag) {
...
/* if (iswap) swap(a, b) */
static void curve25519_swap_conditional(bignum25519 a, bignum25519 b, uint32_t iswap) {
...
static int ge25519_unpack_negative_vartime(ge25519 *r, const unsigned char p[32]) {
	...
	if (!ed25519_compare(check, zero, 32)) {
 ...
 */
static void ed25519_extsk(uint8_t extsk[64], const uint8_t sk[32]) {
	ed25519_hash(sk, 32, extsk);
...
/**
 * compute public key pk from secret key sk
 */
...
/**
 * sign message m with length mlen using keypair sk/pk and place signature in RS
 */
