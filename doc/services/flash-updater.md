...
- get the target `Flash` properties and create a strategy for updating depending on page size, erase size (eg. isn't it faster to compare first, flash only when a difference is found?)
...
- check if there is any firmware already flashed on the target `Flash` (use eg. `object-store` service to determine the version and format based on metadata)
